...
const EditorForm = ({
  ...
  useInput((input, key) => {
    if (key.escape) {
      ...
      key.rightArrow &&
      (currentField === "value" || currentField === "secret")
    ) {
      onEditMode();
    }
  });
  ...
  return (
    ...
    >
      ...
      <Box marginBottom={1}>
        ...
        <Text color={currentField === "value" ? "yellow" : "blue"}>
          {value}
        </Text>
        {currentField === "value" && (
          <Box marginLeft={1}>
            <Text>
              (Press <Text color="yellow">→</Text> to edit{" "}
              {sources[sourceIndex].referenceName})
            </Text>
          </Box>
        )}
      </Box>
      ...
      <Box>
        ...
        {isLoading ? (
          <Text color={currentField === "secret" ? "yellow" : "blue"}>
            Loading...
          </Text>
        ) : (
          <Text color={currentField === "secret" ? "yellow" : "blue"}>
