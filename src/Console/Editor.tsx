...
import { type SecretConfig, type SecretSource, SecretFetchResultType } from "../types.js";
...
interface EditorProps {
	...
	configPath: string;
	onCancel: () => void;
	onSave: (previousName: string | null, name: string, secret: SecretConfig) => void;
}

const Editor = ({ previousName, initialName, secret, sources, configPath, onCancel, onSave }: EditorProps) => {
	const [name, setName] = useState(initialName);
	const [currentField, setCurrentField] = useState<"name" | "source" | "value" | "secret">("name");
				...
				{showSourceEditor ? (
					<Text>
						...
						{currentSource.editorOptions.map((option, index) => (
							<Text key={option.key}>
								<Text color="yellow">{option.key}</Text> to <Text>{option.description}</Text>
					...
					<Text>
						Press <Text color="yellow">←</Text> to save and go back, <Text color="yellow">↑↓</Text> to switch fields,{" "}
