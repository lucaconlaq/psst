...
export const fetchItemsAndFolder = async (path: string): Promise<VaultData> => {
  ...
  try {
    const { stdout: listStdout } = await execAsync(
      `${vaultSource.executable} kv list -format=json ${path}`,
      { encoding: "utf8" }
    );
  ...
  try {
    const { stdout: getStdout } = await execAsync(
      `${vaultSource.executable} kv get -format=json ${path}`,
      { encoding: "utf8" }
    );
...
export const fetchKVMounts = async (
  executable: string
): Promise<string[] | null> => {
  try {
    ...
    const mountsData = JSON.parse(stdout).data.secret as Record<
      string,
      { type: string }
    >;
...
export const fetchSecretFromPath = async (
  pathWithSecret: string
): Promise<string> => {
  const [path, secretKey] = pathWithSecret.split("#");
  if (!path || !secretKey) {
    throw new Error(
      "Invalid path format. Expected format: /path/path/path#secret"
    );
  }
