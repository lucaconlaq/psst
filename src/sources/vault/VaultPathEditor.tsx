...
export const VaultPathEditor = ({ mountPath, onBack, onNavigate, onComplete }: VaultPathEditorProps) => {
	...
	const renderList = () => {
		...
		const visibleFolders = filteredFolders.slice(Math.max(0, startIndex), Math.min(filteredFolders.length, endIndex));
		...
		return (
			<Box flexDirection="column">
				{visibleFolders.map((folder, index) => (
					<Text key={folder} color={startIndex + index === selectedIndex ? "yellow" : undefined}>
						📂 {folder}
					</Text>
				))}
				{visibleItems.map((item, index) => (
					<Text key={item} color={startIndex + filteredFolders.length + index === selectedIndex ? "yellow" : undefined}>
						🔑 {item}{" "}
						<Text color="gray">
							({mountPath}#{item})
						</Text>
