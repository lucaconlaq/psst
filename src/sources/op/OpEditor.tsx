...
export const OpEditor = ({ secret, name, configPath, onComplete }: OpEditorProps) => {
	...
	useEffect(() => {
		try {
			...
			setError("Failed to fetch vaults. Make sure you're logged in to 1Password.");
		} finally {
			setIsLoading(false);
		}
	}, []);
	...
	const loadItems = (vault: Vault) => {
		try {
			const itemsJson = execSync(`${executable} item list --vault "${vault.id}" --format json`, { encoding: "utf8" });
	...
	const loadFields = (item: Item) => {
		try {
			const itemJson = execSync(`${executable} item get "${item.id}" --vault "${item.vault.id}" --format json`, {
				encoding: "utf8",
			});
		...
		return (
			<Box flexDirection="column">
				{visibleRange.map((item, index) => {
					...
					const key = item.id ? `${item.id}-${displayText || index}` : `${displayText}-${index}`;

					return (
						<Text key={key} color={startIndex + index === selectedIndex ? "yellow" : undefined}>
							{emoji}{" "}
							{state === "fields" ? (
								<>
